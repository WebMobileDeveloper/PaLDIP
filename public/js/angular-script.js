		...
		qtSetdata.on('value', function (snapshot) {
			...
			qtdata.on('value', function (snapshot) {
				...
				snapshot.forEach(function (childSnapshot) {
					...
					if (childData['feedqts'] == undefined)
						$scope.questions.push({ key: key, value: childData['question'], type: childData['type'], questionSet: questionSets[set], feedqts: [] });
					else//when scale feedback
						$scope.questions.push({ key: key, value: childData['question'], type: childData['type'], questionSet: questionSets[set], feedqts: childData['feedqts'] });
				});
				$scope.safeApply();
			});
		});
